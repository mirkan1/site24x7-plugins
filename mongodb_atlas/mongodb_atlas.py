        ...
        try:
            ...
            try:
                self.connection = MongoClient(
                    self.connect_string, serverSelectionTimeoutMS=10000)
            ...
            try:
                ...
                data['Extra Info Page Faults per sec'] = per_sec(
                    'extra_info', 'page_faults')
                data['Network Bytes In per sec'] = per_sec(
                    'network', 'bytesIn')
                data['Network Bytes Out per sec'] = per_sec(
                    'network', 'bytesOut')
                data['Network Num Requests per sec'] = per_sec(
                    'network', 'numRequests')
                data['Opcounters Insert per sec'] = per_sec(
                    'opcounters', 'insert')
                data['Opcounters Query per sec'] = per_sec(
                    'opcounters', 'query')
                data['Opcounters Update per sec'] = per_sec(
                    'opcounters', 'update')
                data['Opcounters Delete per sec'] = per_sec(
                    'opcounters', 'delete')
                data['Opcounters Getmore per sec'] = per_sec(
                    'opcounters', 'getmore')
                data['Opcounters Command per sec'] = per_sec(
                    'opcounters', 'command')
                data['Document Deleted per sec'] = per_sec_metric(
                    'metrics', 'document', 'deleted')
                data['Document Inserted per sec'] = per_sec_metric(
                    'metrics', 'document', 'inserted')
                data['Document Returned per sec'] = per_sec_metric(
                    'metrics', 'document', 'returned')
                data['Document Updated per sec'] = per_sec_metric(
            ...
            try:
                delta = (datetime.datetime.utcnow() -
                         output['backgroundFlushing']['last_finished'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mongo_connect_string', help="mongo_connect_string",
                        nargs='?', default=mongo_connect_string)
    parser.add_argument('--dbname', help="dbname",
                        nargs='?', type=str, default=dbname)

    user_args = parser.parse_args()
    mongo_check = MongoDB(user_args)
